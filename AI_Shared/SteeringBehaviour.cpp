...
void SteeringBehaviour::Update(Agent* agent, float deltaTime)
{
	...
	for (ISteeringForce* sf : m_steeringForces)
	{
		//Ask each SteeringForce for its required force
		steeringAcceleration = sf->Update(agent, deltaTime);
		//Multiply by the weighting (defaults to 1)
		steeringAcceleration = Vector2Scale(steeringAcceleration, sf->GetWeighting());
		//Truncated sum of all desired accelerations
		totalAcceleration = Truncate(Vector2Add(totalAcceleration, steeringAcceleration), m_maxAcceleration);
	}

	//Integrate the acceleration to get desired velocity change
