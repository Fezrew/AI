...
void Arrive::Update(float a_deltaTime, Agent& a_agent)
{
	...
	if (Vector2Length(normal) < m_distance)
	{
		...
		Vector2 s = Vector2Scale(normal, (1.0f / (m_distance * a_agent.GetMaxSpeed())));
	...
	else
	{
		...
		normal = Vector2Scale(normal, a_agent.GetMaxSpeed());
	}

	// Calculate the force to add based on the normal vector
	Vector2 force = Vector2Subtract(normal , a_agent.GetVelocity());
